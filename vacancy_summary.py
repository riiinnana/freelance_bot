...
# Названия ролей. Списки живут в `roles.py`, потому что по ним же фильтр
# решает, дизайнера ищут или кого-то другого. Здесь берутся оба разом: в
# заголовке карточки надо назвать должность любой вакансии, даже той,
# которую фильтр потом отклонит.
ROLE_KEYWORDS = ALL_ROLES + ("editor*",)
